...
from .coordinator import WilmaCoordinator, WilmaScheduleCoordinator
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    schedule_coordinator = WilmaScheduleCoordinator(
        hass,
        base_url=entry.data[CONF_BASE_URL],
        username=entry.data[CONF_USERNAME],
        password=entry.data[CONF_PASSWORD],
        children=entry.data[CONF_CHILDREN],
        scan_interval=_opt(CONF_SCHEDULE_SCAN_INTERVAL, DEFAULT_SCHEDULE_SCAN_INTERVAL),
    ...
    hass.data[DOMAIN][f"{entry.entry_id}_schedule"] = schedule_coordinator
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        hass.data[DOMAIN].pop(f"{entry.entry_id}_schedule", None)
    return unload_ok
